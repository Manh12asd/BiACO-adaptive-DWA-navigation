...
public:
    ...
    ACOPlannerActionServer() : Node("aco_planner_action_server") {
        alpha_ = 10.0;
        beta_ = 1.0;
        rho_ = 0.5;
        num_ants_ = 20;
        ...
        RCLCPP_INFO(this->get_logger(), " ACO Planner Ready (Removed Sharp Turn Check).");
    }
...
private:
    ...
    void execute_callback(const std::shared_ptr<GoalHandleAcoPlan> goal_handle) {
        RCLCPP_INFO(this->get_logger(), " Starting ACO path planning...");
        ...
        double final_iteration_best_length = 0.0;
        ...
        if (!start_safe(start_node, local_map)) {
            RCLCPP_ERROR(this->get_logger(), "Start nằm trong vật cản cứng (Cost >= 99)!");
            goal_handle->abort(result);
            return;
        }
        ...
        for (int i = 0; i < iterations_; ++i) {
            ...
            for (int ant = 0; ant < num_ants_; ++ant) {
                ...
                if (pr.success) {
                    ...
                    if (ant == 0) publish_debug_path(pr.full_path, local_map);
                }
            }
            ...
            if (!optimized_paths.empty()) {
                ...
                // Lựa chọn kiến tốt nhất và tệ nhất theo Fit
                ...
                for(size_t k = 0; k < Fit_list.size(); ++k) {
                    if (std::abs(Fit_list[k] - max_fit) <= epsilon) {
                        best_indices.push_back(k);
                    }
                    else if (std::abs(Fit_list[k] - min_fit) <= epsilon) {
                ...
                int current_iteration_best_idx = best_indices[0];

                RCLCPP_INFO(this->get_logger(), 
                            " Iter %d | Kiến Tốt (%zu con, Fit: %.2f) | Kiến Tệ (%zu con, Fit: %.2f)", 
                            i, best_indices.size(), Fit_list[current_iteration_best_idx], worst_indices.size(), min_fit);

                final_iteration_best_chain = optimized_chains[current_iteration_best_idx];
                final_iteration_best_raw_path = raw_paths[current_iteration_best_idx]; 
                final_iteration_best_length = L_list[current_iteration_best_idx];
                
                feedback->iteration = i;
                feedback->current_best_length = final_iteration_best_length; 
                goal_handle->publish_feedback(feedback);

            } else {
                RCLCPP_WARN(this->get_logger(), " Iteration %d: Không tìm thấy đường.", i);
            }
        }
        ...
        if (!final_iteration_best_chain.empty()) {
            ...
            result->total_length = final_iteration_best_length;
            goal_handle->succeed(result);
            
            // Log thời gian chạy ra terminal
            RCLCPP_INFO(this->get_logger(), " Hoàn thành. Độ dài đường đi: %.2fm | Thời gian xử lý ACO: %ld ms", 
                        final_iteration_best_length, duration_ms.count());
        } else {
            RCLCPP_ERROR(this->get_logger(), " Thất bại việc tìm đường sau %ld ms", duration_ms.count());
            goal_handle->abort(result);
        }
    }
};
